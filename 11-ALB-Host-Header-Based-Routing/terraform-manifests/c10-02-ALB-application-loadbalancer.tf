...
module "alb" {
  ...
  https_listener_rules = [
    # Rule-1: app1.mtek.site should go to App1 EC2 Instances
    { 
      ...
      conditions = [{
        #path_patterns = ["/app1*"]
        host_headers = [local.app1_dns_name]
      }]
    },
    # Rule-2: app2.mtek.site should go to App2 EC2 Instances    
    {
      https_listener_index = 0
      actions = [
        {
      ...
      conditions = [{
        #path_patterns = ["/app2*"] 
        host_headers = [local.app2_dns_name]
      }]
    },    
  ]
